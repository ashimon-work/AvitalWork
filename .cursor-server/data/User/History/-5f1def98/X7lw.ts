...
export class HomepageComponent {
  ...
  onCategorySelect(categoryId: string | null): void {
    ...
    if (categoryId) {
      console.log(
        '[HomepageComponent] Filtering products by category:',
        categoryId
      );
        ...
        .pipe(
          map(
            (response: { products: Product[]; total: number }) => {
              console.log(
                '[HomepageComponent] Category products response:',
          ...
          tap((products) =>
            console.log(
              '[HomepageComponent] Category products after mapping:',
              products?.length || 0,
              'products'
            )
          ),
      ...
      console.log('[HomepageComponent] Clearing category filter');
      this.featuredProducts$ = this.apiService.getFeaturedProducts().pipe(
        tap((products) =>
          console.log(
            '[HomepageComponent] Featured products after clear:',
            products?.length || 0,
            'products'
          )
        ),
  ...
  onClearFilter(): void {
    ...
    console.log('[HomepageComponent] Clearing filter - showing all featured products');
    // Re-fetch featured products without category filter
    this.featuredProducts$ = this.apiService.getFeaturedProducts().pipe(
      tap((products) =>
        console.log(
          '[HomepageComponent] Featured products after clear filter:',
          products?.length || 0,
          'products'
        )
      ),
