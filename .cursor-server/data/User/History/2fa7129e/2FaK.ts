...
export class ProductsService {
  ...
  async findAll(
    ...
    if (params.category_id) {
      ...
      // Use subquery approach for ManyToMany filtering - more reliable
      const subQuery = this.productsRepository
        .createQueryBuilder('p')
        .select('p.id')
        .innerJoin('p.categories', 'cat', 'cat.id = :categoryId', {
      ...
      if (params.storeSlug) {
        subQuery
          .innerJoin('p.store', 's')
          .andWhere('s.slug = :storeSlug', { storeSlug: params.storeSlug });
      }
      ...
      this.logger.log(`[ProductsService] Category filter SQL: ${sql}`);
      this.logger.log(`[ProductsService] SQL Parameters: ${JSON.stringify(parameters)}`);
      const [results, total] = await queryBuilder.getManyAndCount();
      this.logger.log(
        `[ProductsService] Found ${total} products for category ${params.category_id}`,
      );
      if (total === 0) {
        this.logger.warn(
          `[ProductsService] No products found for category ${params.category_id}. Checking if products exist...`,
        );
        // Debug: Check if any products exist for this store
        const allProducts = await this.productsRepository.find({
          where: { isActive: true, store: { slug: params.storeSlug } },
          relations: ['categories'],
          take: 5,
        });
        this.logger.log(
          `[ProductsService] Sample products in store (first 5): ${allProducts.length} found`,
        );
        if (allProducts.length > 0) {
          this.logger.log(
            `[ProductsService] First product categories: ${JSON.stringify(
              allProducts[0].categories?.map((c) => ({ id: c.id, name: c.name })),
            )}`,
          );
        }
      }
