...
export class StoreContextGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!storeSlug) {
      console.error('StoreContextGuard: storeSlug parameter missing in request params or query.');
      throw new BadRequestException('Store context (slug) is missing in the request URL (path or query).');
    }
