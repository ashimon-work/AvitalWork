...
export class NotificationsService {
  ...
  async findOne(id: string, userId: string): Promise<NotificationEntity> {
    ...
    if (!notification) {
      throw new NotFoundException(
        `Notification with ID ${id} for user ${userId} not found`,
      );
    }
    return notification;
  }
  ...
  async update(
    ...
    const updatePayload: Partial<NotificationEntity> = {
    ...
    if (!updatedNotification) {
      ...
      throw new NotFoundException(
        `Notification with ID ${id} for user ${userId} not found after update`,
      );
    }
