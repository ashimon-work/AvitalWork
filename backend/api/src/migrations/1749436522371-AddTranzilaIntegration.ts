...
export class AddTranzilaIntegration1749436522371 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "tranzilaTransactionId" character varying(100)`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "orders"."tranzilaTransactionId" IS 'Tranzila transaction/confirmation ID'`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "paymentErrorMessage" text`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "orders"."paymentErrorMessage" IS 'Payment error message if payment failed'`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."orders_paymentstatus_enum" AS ENUM('pending', 'paid', 'completed', 'failed', 'refunded')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_credit_cards_userId" ON "credit_cards" ("userId")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_tranzila_documents_orderId" ON "tranzila_documents" ("orderId")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_tranzila_documents_transactionId" ON "tranzila_documents" ("transactionId")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_orders_tranzilaTransactionId" ON "orders" ("tranzilaTransactionId")`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Remove indexes
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_orders_tranzilaTransactionId"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_tranzila_documents_transactionId"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_tranzila_documents_orderId"`,
    );
    await queryRunner.query(`DROP INDEX IF EXISTS "IDX_credit_cards_userId"`);

    // Remove foreign key constraints
    await queryRunner.query(
      `ALTER TABLE "tranzila_documents" DROP CONSTRAINT IF EXISTS "FK_tranzila_documents_order"`,
    );
    await queryRunner.query(
      `ALTER TABLE "credit_cards" DROP CONSTRAINT IF EXISTS "FK_credit_cards_user"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN IF EXISTS "paymentErrorMessage"`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN IF EXISTS "tranzilaTransactionId"`,
    );
  }
}
