...
export class CreateShippingMethodsTableManually1748137450944
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (table) {
      // Ensure table exists before trying to access its foreignKeys
      const foreignKeyExists = table.foreignKeys.some(
        (fk) =>
          fk.columnNames.indexOf('storeId') !== -1 &&
          fk.referencedTableName === 'stores',
      );
