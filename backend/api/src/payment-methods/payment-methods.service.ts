...
export class PaymentMethodsService {
  ...
  async findAllForUser(
    ...
    this.logger.log(
      `Fetching payment methods for user ${userId} in store ${storeSlug}`,
    );
  ...
  async findOneForUser(
    ...
    if (!paymentMethod) {
      throw new NotFoundException(
        `Payment method with ID "${methodId}" not found for this user and store.`,
      );
    }
  ...
  async createForUser(
    ...
    this.logger.log(
      `Creating payment method for user ${userId} in store ${storeSlug}: ${JSON.stringify(createPaymentMethodDto)}`,
    );
    ...
    const billingAddress = await this.addressRepository.findOne({
      where: {
        id: createPaymentMethodDto.billingAddressId,
        user: { id: userId },
      },
    });
    if (!billingAddress) {
      throw new BadRequestException(
        `Billing address with ID "${createPaymentMethodDto.billingAddressId}" not found or does not belong to the user.`,
      );
    }
    ...
    const newPaymentMethod = this.paymentMethodRepository.create({
      ...
      cardBrand: createPaymentMethodDto.paymentGatewayToken.includes('visa')
        ? 'Visa'
        : 'Mastercard', // Example
    ...
    return this.entityManager.transaction(
      async (transactionalEntityManager) => {
        ...
        try {
          const savedMethod =
            await transactionalEntityManager.save(newPaymentMethod);
          this.logger.log(
            `Payment method ${savedMethod.id} created successfully for user ${userId} in store ${storeSlug}`,
          );
          return savedMethod;
        } catch (error) {
          this.logger.error(
            `Error creating payment method for user ${userId}: ${error.message}`,
            error.stack,
          );
  ...
  async updateForUser(
    ...
    this.logger.log(
      `Updating payment method ${methodId} for user ${userId} in store ${storeSlug}: ${JSON.stringify(updatePaymentMethodDto)}`,
    );

    const paymentMethod = await this.findOneForUser(
      userId,
      storeSlug,
      methodId,
    ); // Ensures method exists and belongs to user/store

    if (updatePaymentMethodDto.billingAddressId) {
      const newBillingAddress = await this.addressRepository.findOne({
        where: {
          id: updatePaymentMethodDto.billingAddressId,
          user: { id: userId },
        },
      });
      if (!newBillingAddress) {
        throw new BadRequestException(
          `New billing address with ID "${updatePaymentMethodDto.billingAddressId}" not found or does not belong to the user.`,
        );
      }
      ...
      async (transactionalEntityManager) => {
        if (paymentMethod.isDefault) {
          ...
          await transactionalEntityManager.update(
            ...
            {
              user: { id: userId },
              store: { id: paymentMethod.store.id },
              id: Not(methodId),
            }, // Exclude current method
            { isDefault: false },
          );
        }
        try {
          ...
          this.logger.log(
            `Payment method ${updatedMethod.id} updated successfully for user ${userId}`,
          );
          return updatedMethod;
        } catch (error) {
          this.logger.error(
            `Error updating payment method ${methodId} for user ${userId}: ${error.message}`,
            error.stack,
          );
          throw new InternalServerErrorException(
  ...
  async deleteForUser(
    ...
    this.logger.log(
      `Attempting to delete payment method ${methodId} for user ${userId} in store ${storeSlug}`,
    );
    const paymentMethod = await this.findOneForUser(
      userId,
      storeSlug,
    ...
    try {
      ...
      this.logger.log(
        `Payment method ${methodId} deleted successfully for user ${userId} in store ${storeSlug}`,
      );
    } catch (error) {
      this.logger.error(
        `Error deleting payment method ${methodId} for user ${userId}: ${error.message}`,
        error.stack,
      );
      throw new InternalServerErrorException(
