...
*   [x] Storefront API Endpoints (Categories, Products, Cart, Orders, Account, **Carousel**, **Newsletter**, etc.) - Endpoints for Products (featured, list, details), Categories (featured, details), Cart (get, add, update, remove), Auth (register), **Carousel (get)**, **Newsletter (subscribe)** implemented. Services refactored to use TypeORM (except Cart). **Product/Category/Carousel endpoints updated to filter by storeSlug.** Search pending.
